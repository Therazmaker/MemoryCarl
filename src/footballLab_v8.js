  ...
  function renderTrackerTab(db){
    ...
    v.innerHTML = `
      <div class="fl-card">
        ...
        <div class="fl-row" style="margin-top:10px;">
          ...
          <input class="fl-input" id="trk_date" type="date" value="${new Date().toISOString().slice(0,10)}">
    ...
    const draw = ()=>{
      ...
      rows.forEach((r,i)=>{
        ...
        if(p>0) cumGain += p;
        if(p<0) cumLoss += p;
        labels.push((r.date && String(r.date).trim()) ? String(r.date) : `#${i+1}`);
    ...
    document.getElementById("trk_add").onclick = ()=>{
      ...
      db.betTracker.push({ id: uid("bet"), date: document.getElementById("trk_date").value || new Date().toISOString().slice(0,10), match: document.getElementById("trk_match").value.trim(), market: document.getElementById("trk_market").value.trim(), result, odds, stake, profit: result==="win" ? (odds-1)*stake : result==="loss" ? -stake : 0 });
      saveDB(db);
      draw();
    };
