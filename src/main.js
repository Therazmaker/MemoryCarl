...
// ===== LIFE TRACKER (TDAH Command Center) =====
...
function getHabitsCompletedToday() {
  ...
  const log = state.lifeTasksLog || [];
  return new Set(log.filter(l => (l.ts || '').slice(0, 10) === todayStr).map(l => l.id)).size;
}

function getLifeTrackerStreak() {
  const log = state.lifeTasksLog || [];
  if(!log.length) return 0;
  const dates = [...new Set(log.map(l => (l.ts || '').slice(0, 10)).filter(Boolean))].sort().reverse();
  ...
  let streak = 0;
  let curr = new Date(dates[0] === today ? today : yesterday);
  for(const d of dates) {
    const expected = curr.toISOString().slice(0, 10);
    if(d === expected) {
      streak++;
...
function getLifeTrackerTdaSuggestion() {
  const tasks = lifeTasksGet().filter(t => !t.isEvent);
  if(!tasks.length) return null;

  const currentHour = new Date().getHours();
  let preferredPart = "cualquier";
  if(currentHour >= 5 && currentHour < 12) preferredPart = "manana";
  else if(currentHour >= 12 && currentHour < 19) preferredPart = "tarde";
  else preferredPart = "noche";

  const scored = tasks.map(task => {
    const days = lifeTaskDaysSince(task);
    const ratio = days / (task.freqDays || 1);
    let score = ratio * 10;
    if(task.partOfDay && (task.partOfDay === preferredPart || task.partOfDay === "cualquier")) {
      score += 5;
    }
    const skippedAt = (state._tdaSkipped && state._tdaSkipped[task.id]) || 0;
    if(Date.now() - skippedAt < 60 * 60 * 1000) {
      score -= 100;
    }
    return { task, score, ratio };
  });
...
function renderLifeTrackerCard() {
  const allItems = lifeTasksGet();
  const tasks = allItems.filter(t => !t.isEvent);
  const events = allItems.filter(t => t.isEvent);
  ...
  const totalAlerts = tasks.filter(t => {
    ...
    if(pendingEvents.length) {
      eventsHtml = `
        <div class="lt-events-section">
          <div class="lt-section-title">📅 Próximos Eventos</div>
          ${pendingEvents.map(evt => {
            const due = new Date(evt.dueDate + "T12:00:00");
            const diffDays = Math.ceil((due - new Date()) / 86400000);
            const dateStr = diffDays === 0 ? "¡Hoy!" : diffDays === 1 ? "Mañana" : diffDays > 1 ? `En ${diffDays} días (${evt.dueDate})` : `Venció (${evt.dueDate})`;
            return `
              <div class="lt-event-row">
                <div class="lt-event-icon">${evt.icon || "💼"}</div>
  ...
  const spotlightTask = getLifeTrackerTdaSuggestion();
  let spotlightHtml = "";
  if(spotlightTask) {
    const urg = lifeTaskUrgency(spotlightTask);
    const daysSince = lifeTaskDaysSince(spotlightTask);
    ...
    spotlightHtml = `
      <div class="lt-spotlight" id="ltSpotlight">
        ...
        <div class="lt-spotlight-sub">Hace ${daysSince === 999 ? "mucho" : daysSince < 1 ? "unas horas" : Math.floor(daysSince) + "d"} (cada ${spotlightTask.freqDays}d)</div>
        <div class="lt-spotlight-actions">
          <button class="lt-spot-btn lt-spot-yes" data-lt-done="${spotlightTask.id}">✓ Listo, lo hice</button>
          <button class="lt-spot-btn lt-spot-later" data-lt-tda-skip="${spotlightTask.id}">Ahora no</button>
          <button class="lt-spot-btn lt-spot-help" data-lt-tda-help="${escapeHtml(spotlightTask.title)}">Micro-paso</button>
        </div>
      </div>
    `;
  } else {
    spotlightHtml = `
      <div class="lt-spotlight lt-spotlight-empty">
        ...
        <div class="lt-spotlight-title">¡Todo despejado!</div>
        <div class="lt-spotlight-sub">No tienes tareas críticas pendientes ahora mismo.</div>
      </div>
    `;
  }
  ...
  if(otherTasks.length) {
    chipRailHtml = otherTasks.map(t => {
      ...
      const isDone = lifeTaskDaysSince(t) < 0.5;
      return `
        <div class="lt-chip lt-chip-${urg} ${isDone ? "lt-chip-done" : ""}" data-lt-chip-id="${t.id}">
          <span>${t.icon || "📌"}</span>
  ...
  let streakBarHtml = `
    <div class="lt-streak-wrap">
      <div class="lt-streak-bar-container">
        <div class="lt-streak-bar-fill ${allDoneToday ? "lt-streak-bar-complete" : ""}" style="width:${totalHabits ? Math.min(100, Math.round((doneToday/totalHabits)*100)) : 0}%"></div>
      </div>
      <div class="lt-streak-info">
        <span class="lt-streak-count">${doneToday}/${totalHabits} hábitos hoy</span>
        ${streak >= 3 ? `<span class="lt-streak-badge">🔥 ${streak} días seguidos</span>` : streak > 0 ? `<span class="lt-streak-badge lt-streak-start">⚡ Racha: ${streak}d</span>` : ""}
      </div>
    </div>
  `;
...
persist = function(){
  ...
  // Auto-sync to Supabase via Vercel API
  try {
    if (window.financePushToSupabase) {
      window.financePushToSupabase();
    }
  } catch(_e) {}
};
